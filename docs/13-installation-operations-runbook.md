...
> 当前实现状态：`v0.13.4` 公开 Beta 已交付生产镜像、四容器 Compose、无需 `.env` 且固定容器名的宝塔单文件入口、通用空节点安装、首位用户 UID 1、`nextbufctl`、首次管理员、备份恢复、跨 Beta 升级和非 Docker 资产。容器/恢复验收由 GitHub Actions 在 Linux amd64/arm64 上执行；Mailpit 只出现在测试覆盖中，不进入生产拓扑。长期决策见 [ADR-0015](./adr/0015-production-packaging-setup-and-recovery.md)、[ADR-0016](./adr/0016-panel-friendly-compose-bootstrap.md) 和 [ADR-0017](./adr/0017-single-file-panel-compose.md)。
