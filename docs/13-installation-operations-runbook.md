...
`STORAGE_LOCAL_PATH` 必须指向持久卷或宿主机持久目录，并与 PostgreSQL 一起备份。当前版本不支持 `STORAGE_DRIVER=s3`；多 Web 实例部署必须等共享存储或 S3 Provider 实现后再启用。
...
5. 配置请求体上限不低于 `AVATAR_MAX_UPLOAD_BYTES`，同时保留合理余量供 multipart 开销；后续通用附件再与对应上传变量协调。
