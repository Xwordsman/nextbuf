...
> 当前实现状态：截至 `v0.11.0`，已实现 Node 形式的 `web`、`worker`、`migrate`、`setup`、`doctor`、`invite`、`mail` 入口、Next.js standalone Web、完整管理后台、站点设置与 Provider 连接诊断，以及 PostgreSQL/Redis/Mailpit 开发/测试 Compose，但尚无生产 NextBuf 镜像、生产四容器 Compose 或 `nextbufctl`。本文仍是 `v0.12.0` 必须完整实现和验证的运维合同；Mailpit 不进入生产拓扑。
