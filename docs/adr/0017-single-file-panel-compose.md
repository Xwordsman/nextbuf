...
两个入口使用同一个不可变应用镜像、相同数据库迁移和运行门禁，不形成第二套应用代码。

### 2. `latest` 只决定拉取通道

正式标签流水线只有在 amd64/arm64 冒烟全部成功后才更新 `latest`。镜像内部仍通过构建参数固化精确应用版本；单文件 Compose 不用环境变量覆盖该版本，因此 preflight、Worker 心跳、备份事实和诊断仍看到真实 SemVer，而不是字符串 `latest`。
...
- 使用 `latest` 的站点获得更低操作成本，同时承担先备份、再拉取和重建的责任。
- 宝塔单实例的四个容器名称稳定且没有 `-1` 后缀；受控 Compose 的扩容能力不受影响。
- 发布测试必须验证单文件模板可以独立解析、只有四个服务、不引用 `.env`、使用正式 `latest` 镜像且具有四个固定容器名。
