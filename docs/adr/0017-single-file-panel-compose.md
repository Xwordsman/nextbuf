...
`v1.0.0` 稳定化补充的 `nextbufctl backup --baota <实际部署编排>` 是两种入口之间的受控桥接，不改变面板仍以内联配置运行的合同。它核对实际四容器、环境和命名卷后生成兼容 `nextbuf-backup-v1` 的原子导出；恢复到受控 Compose 时使用 `--keep-stopped`，先隔离外部 Provider 再启动。日常精确升级、恢复和多实例能力仍属于 `compose.yml + .env`，宝塔模板本身不增加第五个容器或后台守护工具。
...
- 面板实例可以通过显式维护窗口导出到受控备份格式，不再依赖未定义的“面板备份”假设。
- 面板 Compose 中包含秘密，文件权限、截图和备份必须按敏感配置处理。
- 使用 `latest` 的站点获得更低操作成本，同时承担它只跟随最新完整稳定 Release、升级前仍应先备份再拉取和重建的责任；Beta 候选只发布到 `edge` 与不可变 `sha-*`，不会提升 `latest`。
