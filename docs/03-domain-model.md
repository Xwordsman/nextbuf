...
- 置顶和精华是明确管理标志；热门不落库，当前按 30 天活动窗口及回复数/浏览数阈值计算。
- 首页和节点流只公开 `published/closed`，按置顶、最后活动时间和主题编号稳定排序，使用包含这三个排序键的 opaque cursor 前后翻页。
- 浏览量字段已经建立但 `v0.6.0` 不按每次 HTTP 请求直接增长；反滥用浏览统计在互动阶段接入。
...
`v0.6.0` 创建主题时在同一 PostgreSQL 事务中创建 Topic、position=1 的首帖和 version=1 修订。普通回复尚未开放。主题级删除、隐藏和恢复只同步首帖状态，不批量覆盖未来普通回复自己的删除状态。

### Revision

主题和回复编辑后保存修订：编辑者、编辑时间、来源、修改原因以及必要的内容快照。自动格式化等无意义变化可合并，避免无限产生修订。

当前每次标题或正文实际变化都会增加单调版本号，保存标题和正文快照、编辑者、来源和时间；仅节点移动或状态管理不会制造伪内容版本。主题详情显示修订次数，作者/版主编辑页显示最近修订记录。

### 内容格式

V1 建议保存 Markdown 源内容。渲染时执行允许列表清洗，默认禁止原始 HTML、危险 URL 协议、脚本和内联事件。渲染缓存可以重建，源内容是事实来源。

`v0.6.0` 只按纯文本保存和展示正文，保留换行并由 React 转义，不把尚未实现的 Markdown 解析器伪装成已完成能力。Markdown、附件和安全渲染管线属于 `v0.7.0`。
