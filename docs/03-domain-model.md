...
- `v0.13.8` 为作者的新建/编辑会话保存 nullable UUID `editorSessionKey` 和单调 `editorSessionRevision`。同一作者的非空 key 唯一；历史数据保持 `NULL/NULL`。同 key 重试只能推进更高 revision，发布终态不能被迟到自动保存改回草稿。
...
`v0.7.0` 已开放普通回复：Topic 的 `nextPostPosition` 在 Topic 行锁内单调分配，数据库唯一约束兜底，同一 Topic 的回复从 position=2 开始且永不复用。首帖固定为 `position=1`；面向成员显示的回复楼层等于内部位置减 1，因此第一条回复显示为 `#1`，而稳定的永久锚点仍使用内部 `#post-<position>`。回复创建同时写入初始修订、提及、附件引用、有效回复计数和审计；每用户每小时最多 20 条，由锁定用户行的 PostgreSQL 事务限制。回复支持同 Topic 引用、编辑、软删除和恢复，删除只减少有效回复数并保留楼层。回复列表每页读取 30 个实际 Post，包括删除墓碑。

### EditorSession

`v0.13.8` 的编辑会话是网络重试身份，不是登录 Session，也不替代 Better Auth。浏览器只保存随机 UUID、revision 和页面作用域；正文仍以 PostgreSQL Draft/Post 为事实。

- Topic 直接保存作者/key/revision，同一作者/key 只对应一个 Topic。
- 回复最终 Post 与可见 ReplyDraft 镜像保存 key/revision；`CommunityReplyEditorSession` 额外保存 `active`、`cleared`、`published`、`superseded` 状态和可选最终 Post。
- `cleared` 保留已清空草稿的 revision 终点，`superseded` 表示旧 key 已被新会话接管，`published` 让响应丢失后的重试解析到同一楼层。恢复到 `superseded` 时客户端清除旧 history 会话并重新加载规范 Topic 页面状态，而不是重复恢复。
- 写入统一锁定用户行再锁定 Topic 行；同 revision 不同快照和较低 revision 返回冲突。客户端遇到冲突后冻结写入并要求刷新，不自动覆盖另一个标签页。
- 每个用户滚动一小时最多创建 60 个新回复编辑会话；已有 key 的重试不计为新会话。`cleared`/`superseded` 墓碑保留 30 天，由 Worker 每批最多清理 500 条过期记录；`active`/`published` 不参与该清理。
- 写请求超时或收到响应体无效的 2xx 都只能判定为结果未知，客户端必须使用原 key 恢复，不能换 key 直接重发。
- revision 上限是 PostgreSQL `INTEGER` 上限 `2147483647`。详细状态机、恢复和回退见 [ADR-0019](./adr/0019-editor-autosave-idempotency-and-draft-privacy.md)。
...
- 每个用户在每个 Topic 最多一份可见回复草稿；客户端严格串行自动保存，显式发布取消 debounce 并等待在途保存。清空正文会删除 Draft/附件引用但保留 `cleared` 会话终点；发布事务删除 Draft 并把会话绑定到最终 Post。Topic 关闭或作者当前受限时，作者仍可删除自己已经存在的回复草稿，删除能力不随发布权限一起丢失；其他写入继续按各自权限判断。站点 `repliesEnabled=false` 只阻止正式发布，仍允许保存和编辑草稿。
