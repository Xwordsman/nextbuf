...
Compose 中还可以出现一次性的 `setup` 工具和可选备份服务。NextBuf 最初将 setup 作为默认一次性服务，但部分面板会把正常的 `Exited(0)` 误报为整个项目停止；当前默认单机编排改由 Web 启动阶段执行幂等 setup，显式 setup 只在工具 profile 中临时运行，仍不计入常驻容器数量。
