...
> 当前实现状态：`v0.13.10` 候选让共享 Zod Schema、根 `.env.example`、生产 Compose、宝塔单文件 Compose、Web、Worker、CLI、setup、doctor 和发布包继续使用同一合同。可信代理只接受明确 IP/CIDR，非 Docker Web 固定绑定回环；站点名称、注册策略、发布开关和每小时限额由 PostgreSQL `site_settings` 管理，Provider 密钥不在线保存或回显。
