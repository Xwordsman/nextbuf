...
> 当前实现状态：截至 `v0.11.0`，`.env.example` 中的应用、数据库、Redis、Worker、认证、SMTP、GitHub OAuth 和本地/S3 存储变量已由共享 Zod Schema 实现。站点名称、注册策略、发布开关和每小时限额由 PostgreSQL `site_settings` 单例管理；后台只返回 Provider 脱敏状态并执行服务端连接测试，不在线保存或回显完整部署密钥。
...
`v0.11.0` 已将下列运营设置保存在数据库后台，而不是继续增加环境变量：
