...
> 当前实现状态：`v0.13.2` 已让共享 Zod Schema、根 `.env.example`、生产 Compose、宝塔单文件 Compose、Web、Worker、CLI、setup、doctor 和发布包使用同一合同。站点名称、注册策略、发布开关和每小时限额继续由 PostgreSQL `site_settings` 管理；Provider 密钥不在线保存或回显。
...
`compose.baota.yml` 不读取 `.env`，也不需要 `NEXTBUF_IMAGE`、`NEXTBUF_VERSION` 或 `NEXTBUF_ENV_FILE`。它将镜像固定为正式 `latest` 通道，并把应用、PostgreSQL 和 Redis 所需值直接写在同一个 Compose。重复出现的数据库/Redis 密码必须保持一致；所有 `replace-` 值、示例域名和示例 `MAIL_PAYLOAD_KEY` 都必须在首次启动前替换。镜像内部的精确 `NEXTBUF_VERSION` 仍由发布构建写入，不能在面板模板中覆盖。
