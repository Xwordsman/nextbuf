...
> 当前实现状态：`v0.13.0` 公开 Beta 已交付生产打包、首次管理员、备份恢复、精确升级、容量诊断、故障注入和跨 Beta 镜像升级门槛。Mailpit 只用于开发、测试和 CI，不是第五个生产容器。生产与恢复合同见 [ADR-0015](./adr/0015-production-packaging-setup-and-recovery.md)，面板启动协调见 [ADR-0016](./adr/0016-panel-friendly-compose-bootstrap.md)。
...
- `0.13.0-amd64`、`0.13.0-arm64`：标签流水线内部使用的单架构候选；两个架构冒烟全部通过后才合并为正式版本。
- `latest`：最新稳定版的可变别名，只在正式标签发布成功后更新。
