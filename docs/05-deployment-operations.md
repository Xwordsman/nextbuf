...
> 当前实现状态：截至 `v0.10.0`，已提供 PostgreSQL/Redis/Mailpit 开发与测试 Compose、Web/Worker/migrate/setup/doctor/invite/mail 入口、Next.js standalone Web、Better Auth、SMTP/附件/浏览/通知/信任重算 Worker、持久化失败重放和周期调度、本地/S3 对象存储、真实社区互动、治理与 PostgreSQL 搜索；尚未提供生产应用镜像和四容器生产 Compose。本文其余发布流程仍是 `v0.12.0` 必须实现的合同。Mailpit 只用于开发、测试和 CI，不是第五个生产容器。
