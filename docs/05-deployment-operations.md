...
> 当前实现状态：`v0.13.8` 是最后一个完整发布的公开 Beta。`v0.13.9` 标签流水线因 Linux x64 standalone 归档中的 pnpm 依赖链接被展平而无法启动 Web；该标签与已生成镜像保持不可变，但不构成完整、受支持的发布，也不能设为公开升级基线。替代候选 `v0.13.10` 不新增迁移或产品功能，原样保留 pnpm 符号链接并验证链接不悬空、不逃出归档及解压产物完整启动；它尚未发布，必须先通过精确 `v0.13.8 -> v0.13.10` 升级、恢复、双架构镜像和归档门槛。生产与恢复合同见 [ADR-0015](./adr/0015-production-packaging-setup-and-recovery.md)，面板启动协调见 [ADR-0016](./adr/0016-panel-friendly-compose-bootstrap.md)，单文件面板入口见 [ADR-0017](./adr/0017-single-file-panel-compose.md)。Mailpit 只用于开发、测试和 CI，不是第五个生产容器。
...
- `0.13.6`：公开首页迁移到官方 shadcn/ui，同时保留真实 PostgreSQL 数据、既有三栏宽度和节点页外观；不改变数据库、配置、部署或授权合同。
- `0.13.7`：将节点、主题、编辑器、账户、认证、安装、搜索、通知、状态和公共壳层统一迁移到官方 shadcn/ui；不改变数据库、配置、部署或授权合同。
...
- `0.13.10`：替代最终 Beta 候选，迁移和产品功能与 `0.13.9` 相同；归档原样保留 pnpm 符号链接并拒绝悬空或越界链接，且必须从解压产物完整启动 Web/Worker。候选尚未发布，发布前必须通过精确 `0.13.8 -> 0.13.10` 升级与全部交付门槛。
...
- 每分钟维护任务会扫描回复编辑会话：只删除 `updated_at` 已超过 30 天的 `cleared`/`superseded` 墓碑，每次最多 500 条，并把删除数写入最近任务结果。`active` 草稿与 `published` 终态不参与该清理；不需要额外环境变量或人工定时任务。
