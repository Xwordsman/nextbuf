...
> 当前实现状态：截至 `v0.6.0`，已提供 PostgreSQL/Redis/Mailpit 开发与测试 Compose、Web/Worker/migrate/setup/doctor/invite 入口、Next.js standalone Web、Better Auth、SMTP Worker、本地头像存储和真实节点/主题持久化；尚未提供生产应用镜像和四容器生产 Compose。本文其余发布流程仍是 `v0.12.0` 必须实现的合同。Mailpit 只用于开发、测试和 CI，不是第五个生产容器。
