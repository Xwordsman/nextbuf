...
> 当前实现状态：`v0.12.0` 已交付根目录生产 `Dockerfile`、`compose.yml`、`.env.example`、`nextbufctl`、首次管理员、非 Docker 发布包、systemd/PM2/Nginx 示例和 GitHub Actions 多架构发布。Mailpit 只用于开发、测试和 CI，不是第五个生产容器。持久合同见 [ADR-0015](./adr/0015-production-packaging-setup-and-recovery.md)。
...
根目录 [`compose.yml`](../compose.yml) 是正式服务关系。核心结构如下；部署必须使用 Release 中与镜像版本匹配的完整文件：
...
`NEXTBUF_IMAGE` 默认是 `ghcr.io/xwordsman/nextbuf`，`NEXTBUF_VERSION` 必须是精确版本。私有镜像镜像站可以覆盖地址，但不得让 Web 与 Worker 使用不同版本。
...
5. 主分支发布 `edge`/SHA 标签，版本标签发布 GHCR 精确 SemVer。
6. 生成校验和、SBOM、版本说明和非 Docker 发布包。
7. `v*` 标签生成 GitHub Release、非 Docker tar.gz、SHA-256 和 SPDX JSON SBOM。
