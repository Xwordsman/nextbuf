...
> 当前实现状态：截至 `v0.3.0`，已提供 PostgreSQL/Redis 开发与测试 Compose、Web/Worker/migrate/setup/doctor 入口，以及可直接运行的 Next.js standalone Web 产物；尚未提供生产应用镜像和四容器生产 Compose。本文其余发布流程仍是 `v0.12.0` 必须实现的合同。
