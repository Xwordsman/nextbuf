...
> 当前阶段：`v0.11.0` 管理后台与站点配置已完成，下一阶段为 `v0.12.0` 安装、Docker、Actions 和运维。`UI/index.html` 仅是历史视觉参考，运行页面以 `src/app` 和 `src/components` 为准。
...
当前已有可运行的 Next.js Web、Worker、CLI、开发/测试 Compose、身份认证、账号资料、真实节点/主题/回复、Markdown、本地/S3 附件、互动、浏览聚合、热门排序、PostgreSQL 搜索、通知/偏好/普通邮件、治理案件、制裁授权及可恢复信任重算链路，但仍没有生产 Dockerfile、生产 Compose 或正式发布包。文档中标注为未来合同的命令必须在对应版本实现后才能使用。
