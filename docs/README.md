...
> 当前阶段：`v0.13.5` 公开 Beta 补丁已将管理后台迁移至官方 shadcn/ui 组件体系，并保持无需 `.env`、固定容器名的宝塔单文件 Compose、工作流化后台内容管理和 UID 1 安装合同；下一步仍是按验收模板执行真实服务器和邀请用户验收，不自动开始 `v1.0.0`。`UI/index.html` 仅是历史视觉参考，运行页面以 `src/app` 和 `src/components` 为准。
