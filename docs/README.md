...
> 当前阶段：`v0.13.8` 公开 Beta 候选在 `v0.13.7` 官方 shadcn/ui 全站界面上修复编辑器幂等、刷新恢复和私人草稿边界，并保持真实社区数据、既有三栏布局、无需 `.env`、固定容器名的宝塔单文件 Compose、工作流化后台内容管理和 UID 1 安装合同；下一步是候选 CI、`v0.13.7` 升级及真实服务器邀请用户复验，不自动开始 `v1.0.0`。`UI/index.html` 仅是历史视觉参考，运行页面以 `src/app` 和 `src/components` 为准。
