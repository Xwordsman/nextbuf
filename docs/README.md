...
> 当前阶段：`v0.13.2` 公开 Beta 补丁已增加无需 `.env` 的宝塔单文件 Compose，并保留通用空节点安装和首次访问引导；下一步仍是按验收模板执行真实服务器和邀请用户验收，不自动开始 `v1.0.0`。`UI/index.html` 仅是历史视觉参考，运行页面以 `src/app` 和 `src/components` 为准。
