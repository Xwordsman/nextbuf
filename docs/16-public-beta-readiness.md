...
当前追加补丁为 `v0.13.8`：它关闭邀请测试中发现的编辑器自动保存/发布竞态和后台私人草稿泄漏风险。主分支与正式标签已经通过本文第 15 节的新增门槛，[GitHub Actions 发布验证](https://github.com/Xwordsman/nextbuf/actions/runs/29733660199)、[GitHub Release](https://github.com/Xwordsman/nextbuf/releases/tag/v0.13.8) 和不可变双架构 GHCR `0.13.8` 均已发布；同一提交的滚动 `latest` 已在真实服务器拉取复验通过。

`v0.13.8` 仍是最后一个完整发布版本和当前公开升级基线。不可变 `v0.13.9` 标签已通过双架构镜像门槛，但 Linux x64 standalone 归档中的 pnpm 依赖链接被展平，导致 Web 无法启动，Release 资产未完成；该标签与镜像保持历史原状，不构成完整、受支持的发布，也不能提升升级基线。历史细节见 [`v0.13.9` 发布说明](./18-v0.13.9-release-notes.md)。

替代最终 Beta 候选为 `v0.13.10`，当前尚未发布。它不新增迁移或产品功能，继续关闭可信代理、隐藏节点附件、依赖公告、非 Docker 归档身份、备份一致性和失败升级语义，并原样保留 standalone 的 pnpm 符号链接。候选必须拒绝悬空或逃出归档的链接，从精确 `v0.13.8` 镜像保留真实管理员、主题、回复、草稿和附件，完成空卷恢复、故障注入、双架构镜像与解压归档完整启动后才发布；具体候选门槛见 [`v0.13.10` 发布说明](./18-v0.13.10-release-notes.md)。
...
`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 保存已发布 Beta 迁移基线；`v0.13.8.json` 冻结编辑会话追加迁移及此前全部迁移的顺序与 SHA-256。`v0.13.9.json` 保存相同 13 条迁移的历史标签清单，但 `v0.13.9` 发布不完整，因此该文件不代表公开升级基线；替代候选 `v0.13.10.json` 继续冻结完全相同的 13 条迁移，证明修复归档没有新增或重写迁移。校验器强制要求与当前 `package.json` 版本同名的清单存在，并完整覆盖仓库当前迁移全集，不能靠遗漏候选清单获得绿色结果。执行：

```bash
pnpm migration:verify
```

校验会拒绝删除、重排或修改任何已冻结迁移；当前版本清单还必须覆盖所有追加迁移。该静态门槛不能替代真实升级；`v0.13.8` 发布流水线已经从精确 `v0.13.7` 镜像创建持久数据、备份并升级到发布镜像。
...
定时、手动、正式标签，以及源码版本不同于当前公开升级基线的主分支流水线，都会在 amd64 候选镜像上执行真实 Compose 故障演练：
...
源码版本等于当前公开升级基线的普通主分支提交不执行这组慢速故障演练，但仍会在 amd64/arm64 上执行 Web/Worker/数据库/Redis/首次管理员基础镜像冒烟；两个架构成功后才更新滚动 `latest`。候选版本一旦前进，即使迁移集合没有变化，主分支也必须先完成故障和恢复门槛。生产环境的托管 PostgreSQL、Redis、SMTP 和 S3 仍需部署者按运行手册完成实例级告警和恢复演练。
...
每日定时、手动和正式 `v0.13.x` 标签都会在 amd64 流水线执行当前受支持 Beta 升级。主分支不是只比较相邻两次提交：只要源码版本不同于最后公开升级基线，每个后续候选提交都必须重新通过空卷恢复、故障注入和真实升级，避免前一次流水线被取消或后续只改代码时绕过门槛。该判断基于版本而不是迁移差异；当前发布门槛是精确 `v0.13.8 -> v0.13.10`，即使两版迁移树相同也必须执行：

1. 拉取公开升级基线指定的精确 `ghcr.io/xwordsman/nextbuf:<基线版本>` 镜像；当前为 `0.13.8`。
...
5. 验证目标版本接口、管理员、安装完成状态、上述持久内容、历史编辑字段 `NULL/NULL`、完整迁移集合和 `runtime.initialized` 目标版本。
6. 再执行完整 doctor；流水线结束后销毁临时 Registry、Compose 卷和备份。

候选镜像的空卷恢复由上述版本不同的主分支、定时、手动和标签流水线的 `nextbuf-backup-v1` 恢复测试覆盖。迁移成功后仍不承诺旧代码直接读取新 Schema；失败恢复遵循 ADR-0015。

迁移版本正式标签、镜像和 Release 全部成功后，后续开发提交必须把工作流的公开升级基线提升到该已发布版本；不能在标签验证前提前提升。`v0.13.8` 标签验证已经通过，工作流公开升级基线现为 `v0.13.8`。

历史证据：CI #56 已真实完成同类六步并验证升级后的完整 doctor；正式标签 CI #58 已再次通过当时的升级门槛，并完成标签镜像、manifest、SBOM、provenance、归档和 GitHub Release 验证。`v0.13.9` 标签流水线虽完成双架构镜像，却在 Linux x64 归档启动阶段失败，不能作为完整发布或升级基线证据，且不可通过移动标签补写历史结果。`v0.13.10` 必须由新的主分支和正式标签流水线产生完整证据，当前尚未发布。
...
3. 当前候选相对公开 `v0.13.7` 标签仍有迁移差异时，每个 `main` 提交在更新 `latest` 前都必须额外完成精确 `v0.13.7 -> v0.13.8` 备份/升级，保留既有管理员、节点、主题、首帖、回复、回复草稿、附件引用和 `NULL/NULL` 历史行。
