...
`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 分别保存两个已发布 Beta 基线的迁移顺序与 SHA-256。执行：

```bash
pnpm migration:verify
```

校验会拒绝删除、重排或修改任何已冻结迁移，但允许在最新基线之后只追加新迁移。该静态门槛不能替代真实升级；`v0.13.1` 仍需从精确 `v0.12.0` 镜像创建数据、备份、升级到候选镜像并执行恢复演练，同时验证空安装移除旧演示节点而已有站点保留节点。
...
每日定时、手动和正式 `v0.13.x` 标签都会在 amd64 流水线执行首个受支持 Beta 升级；普通主分支提交只执行发布滚动 `latest` 所需的双架构基础冒烟：
