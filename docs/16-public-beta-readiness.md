...
当前追加候选为 `v0.13.8`：它关闭邀请测试中发现的编辑器自动保存/发布竞态和后台私人草稿泄漏风险。该候选包含追加迁移，必须通过本文第 15 节的新门槛后才能发布或更新 `latest`；此前 `v0.13.7` 仍是最后已发布版本。
...
校验会拒绝删除、重排或修改任何已冻结迁移；当前版本清单还必须覆盖所有追加迁移。该静态门槛不能替代真实升级；`v0.13.8` 仍需从精确 `v0.13.7` 镜像创建持久数据、备份并升级到候选镜像。
...
迁移版本正式标签、镜像和 Release 全部成功后，后续开发提交必须把工作流的公开升级基线提升到该已发布版本；不能在标签验证前提前提升。`v0.13.8` 候选期间基线保持 `v0.13.7`。
