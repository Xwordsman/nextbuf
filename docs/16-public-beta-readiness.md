...
`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 保存已发布 Beta 基线；`v0.13.8.json` 冻结本次编辑会话追加迁移及此前全部迁移的顺序与 SHA-256。校验器强制要求与当前 `package.json` 版本同名的清单存在，并完整覆盖仓库当前迁移全集，不能靠遗漏候选清单获得绿色结果。执行：
...
普通主分支提交不执行这组慢速故障演练，但会在 amd64/arm64 上执行 Web/Worker/数据库/Redis/首次管理员基础镜像冒烟；两个架构成功后才更新滚动 `latest`。生产环境的托管 PostgreSQL、Redis、SMTP 和 S3 仍需部署者按运行手册完成实例级告警和恢复演练。
...
每日定时、手动和正式 `v0.13.x` 标签都会在 amd64 流水线执行当前受支持 Beta 升级。主分支不是只比较相邻两次提交：只要当前候选相对最后公开标签仍包含迁移差异，每个后续候选提交都必须重新通过升级，避免前一次流水线被取消或后续只改代码时绕过门槛；迁移树与公开标签相同的普通提交只执行双架构基础冒烟：

1. 拉取公开的精确 `ghcr.io/xwordsman/nextbuf:0.13.7` 基线镜像。
2. 把基线和本次已构建的候选镜像发布到 Runner 内的临时 Registry，确保 `nextbufctl upgrade` 走真实 pull 流程。
3. 在 `v0.13.7` 创建首位管理员、自定义节点、主题、首帖、回复、回复草稿、本地附件和当前/修订/草稿三类附件引用。
4. 由 `nextbufctl` 自动创建升级前备份，停止 Web/Worker，并运行目标镜像的幂等 `setup`。
5. 验证目标版本接口、管理员、安装完成状态、上述持久内容、历史编辑字段 `NULL/NULL`、新迁移和 `runtime.initialized` 目标版本。
6. 再执行完整 doctor；流水线结束后销毁临时 Registry、Compose 卷和备份。

候选镜像的空卷恢复继续由定时、手动和标签流水线的 `nextbuf-backup-v1` 恢复测试覆盖。迁移成功后仍不承诺旧代码直接读取新 Schema；失败恢复遵循 ADR-0015。
