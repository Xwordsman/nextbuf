...
`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 保存已发布 Beta 迁移基线；`v0.13.8.json` 冻结编辑会话追加迁移及此前全部迁移的顺序与 SHA-256。`v0.13.9.json` 保存相同 13 条迁移的历史标签清单，但 `v0.13.9` 发布不完整，因此该文件不代表公开升级基线；已发布 `v0.13.10.json` 继续冻结完全相同的 13 条迁移，证明修复归档没有新增或重写迁移。校验器强制要求与当前 `package.json` 版本同名的清单存在，并完整覆盖仓库当前迁移全集，不能靠遗漏候选清单获得绿色结果。执行：

```bash
pnpm migration:verify
```

校验会拒绝删除、重排或修改任何已冻结迁移；当前版本清单还必须覆盖所有追加迁移。该静态门槛不能替代真实升级；`v0.13.10` 主线与标签流水线已经从精确 `v0.13.8` 镜像创建持久数据、备份并升级到发布镜像。
...
每日定时、手动和正式版本标签都会在 amd64 流水线执行当前受支持升级。主分支不是只比较相邻两次提交：只要源码版本不同于最后公开升级基线，每个后续候选提交都必须重新通过空卷恢复、故障注入和真实升级，避免前一次流水线被取消或后续只改代码时绕过门槛。该判断基于版本而不是迁移差异；`v0.13.10` 已完成精确 `v0.13.8 -> v0.13.10` 发布门槛，当前基线已经提升到 `0.13.10`，后续 `v1.0.0` 候选必须执行精确 `v0.13.10 -> v1.0.0`：
