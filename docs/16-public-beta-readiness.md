...
最终 Beta 候选为 `v0.13.9`：它不新增迁移或产品功能，集中关闭可信代理、隐藏节点附件、依赖公告、非 Docker 归档身份、备份一致性和失败升级语义。候选必须从精确 `v0.13.8` 镜像保留真实管理员、主题、回复、草稿和附件，完成空卷恢复、故障注入、双架构镜像与解压归档启动后才发布；主分支也必须先构建并启动同版本 Linux x64 归档，标签不再是首次执行该门槛。具体升级和恢复边界见 [发布说明](./18-v0.13.9-release-notes.md)。
...
`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 保存已发布 Beta 基线；`v0.13.8.json` 冻结编辑会话追加迁移及此前全部迁移的顺序与 SHA-256；`v0.13.9.json` 冻结完全相同的 13 条迁移，证明最终 Beta 加固没有新增或重写迁移。校验器强制要求与当前 `package.json` 版本同名的清单存在，并完整覆盖仓库当前迁移全集，不能靠遗漏候选清单获得绿色结果。执行：
...
每日定时、手动和正式 `v0.13.x` 标签都会在 amd64 流水线执行当前受支持 Beta 升级。主分支不是只比较相邻两次提交：只要源码版本不同于最后公开升级基线，每个后续候选提交都必须重新通过空卷恢复、故障注入和真实升级，避免前一次流水线被取消或后续只改代码时绕过门槛。该判断基于版本而不是迁移差异，因此 `v0.13.8 -> v0.13.9` 的迁移树即使相同也必须执行：
...
历史证据：CI #56 已真实完成同类六步并验证升级后的完整 doctor；正式标签 CI #58 已再次通过当时的升级门槛，并完成标签镜像、manifest、SBOM、provenance、归档和 GitHub Release 验证。`v0.13.9` 的候选与标签证据必须在对应流水线成功后另行记录，不能借用这些历史运行。
