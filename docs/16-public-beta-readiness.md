...
日常主分支提交不执行这组慢速故障演练，只在 amd64/arm64 上执行 Web/Worker/数据库/Redis/首次管理员基础镜像冒烟；每个架构在冒烟前固定运行时 Digest，成功后上传同时绑定候选顶层 Digest 的短期 artifact。两个架构成功且提交仍是远程 HEAD 后，工作流只从这些内容地址合并 `edge` 与不可变 `sha-*`；正式标签合并候选顶层 index，并验证运行时平台、全部描述符和 SBOM/provenance attestation 关联均未丢失。主线不移动 `latest`。候选必须先由定时或手动运行取得故障和恢复证据，正式标签再重跑同一深度门槛。生产环境的托管 PostgreSQL、Redis、SMTP 和 S3 仍需部署者按运行手册完成实例级告警和恢复演练。
...
每日定时、手动和正式版本标签都会在 amd64 流水线执行当前受支持升级；日常主分支只执行基础双架构镜像冒烟，不运行空卷恢复、故障注入或跨版本升级。候选在正式标签前必须至少通过一次定时或手动深度运行，正式标签再完整重跑，避免只凭日常主线结果进入稳定通道。`v0.13.10` 已完成精确 `v0.13.8 -> v0.13.10` 发布门槛，当前基线已经提升到 `0.13.10`，后续 `v1.0.0` 候选必须执行精确 `v0.13.10 -> v1.0.0`。升级前先确认至少 1 位可接管管理员，生产实例优先 2 位或以上；可接管条件为 `admin/site`、active、邮箱已验证、无注销申请/计划、无有效 suspend/ban 且有密码非空的 `credential` Account：
...
3. 当时的公开 Beta 流水线要求迁移有差异的 `main` 提交在更新滚动 `latest` 前完成精确 `v0.13.7 -> v0.13.8` 备份/升级，保留既有管理员、节点、主题、首帖、回复、回复草稿、附件引用和 `NULL/NULL` 历史行；该历史门槛已经通过。当前稳定发布通道由 ADR-0020 接管：日常 `main` 更新 `edge`，深度恢复、故障注入和跨版本升级在定时、手动及正式标签运行中执行，只有完整稳定 Release 更新 `latest`。
