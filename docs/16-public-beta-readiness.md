...
`v0.13.8` 关闭邀请测试中发现的编辑器自动保存/发布竞态和后台私人草稿泄漏风险。主分支与正式标签已经通过本文第 15 节的新增门槛，[GitHub Actions 发布验证](https://github.com/Xwordsman/nextbuf/actions/runs/29733660199)、[GitHub Release](https://github.com/Xwordsman/nextbuf/releases/tag/v0.13.8) 和不可变双架构 GHCR `0.13.8` 均已发布；同一提交的滚动 `latest` 已在真实服务器拉取复验通过。

不可变 `v0.13.9` 标签已通过双架构镜像门槛，但 Linux x64 standalone 归档中的 pnpm 依赖链接被展平，导致 Web 无法启动，Release 资产未完成；该标签与镜像保持历史原状，不构成完整、受支持的发布，也没有提升升级基线。历史细节见 [`v0.13.9` 发布说明](./18-v0.13.9-release-notes.md)。

`v0.13.10` 已作为替代最终 Beta 完整发布。它不新增迁移或产品功能，关闭可信代理、隐藏节点附件、依赖公告、非 Docker 归档身份、备份一致性和失败升级语义，并原样保留 standalone 的 pnpm 符号链接。[主线 CI](https://github.com/Xwordsman/nextbuf/actions/runs/30470438423) 与 [标签 CI](https://github.com/Xwordsman/nextbuf/actions/runs/30518120224) 已拒绝悬空或逃出归档的链接，从精确 `v0.13.8` 镜像保留真实管理员、主题、回复、草稿和附件，并完成空卷恢复、故障注入、双架构镜像与解压归档完整启动；[GitHub Release](https://github.com/Xwordsman/nextbuf/releases/tag/v0.13.10) 和不可变 GHCR `0.13.10` 已发布。具体边界见 [`v0.13.10` 发布说明](./18-v0.13.10-release-notes.md)。
...
| 数据损坏 | 事务、数据库约束、稳定楼层、不可变修订、编辑 UUID/revision、回复会话终态、Outbox、幂等 Worker、可验证备份 | 真实服务集成测试、编辑竞态 E2E、Compose setup/restore/upgrade smoke | S3 对象需要 Provider 级版本/快照；迁移成功后不承诺盲目代码降级 |
...
本节保留 Beta 阶段的来源证据；`v1.0.1` 已发布，当前性能补丁的增量判据以 [`v1.0.2` 发布就绪清单](./26-v1.0.2-release-readiness.md)为准。
...
- `v1.0.0` 已实现并发布最终账号注销与管理员连续性。升级前至少需要 1 位合格的可接管管理员，正式发布默认 2 位或以上；合格条件为 `admin/site`、active、已验证、未申请或计划注销、无有效 suspend/ban 且有非空 credential 密码。单版本风险豁免不得自动延伸，当前 `v1.0.2` 规则见专用清单。
...
- 幂等合同按编辑会话 UUID 判定；两个互不共享 key 且都明确提交的独立编辑会话不会按正文相似度自动合并。
- `cleared`/`superseded` 对迟到写入的墓碑保护窗口是最近更新后的 30 天；正常客户端恢复到 `superseded` 会清除旧 history，`active`/`published` 会话不参与定期清理。
...
- 管理员连续性：0 位可接管管理员为失败，1 位为冗余警告，2 位及以上为健康。

后台首页将无就绪 Worker 或队列不可用标为严重，将 Queue/Outbox 积压和持久失败标为警告。默认警戒值是等待 Queue 或待发布 Outbox 达到 500；这是早期运营告警，不等于硬容量上限。

## 11. 迁移历史与升级证据

`prisma/migration-baselines/v0.12.0.json` 与 `v0.13.0.json` 保存已发布 Beta 迁移基线；`v0.13.8.json` 冻结编辑会话追加迁移及此前全部迁移的顺序与 SHA-256。`v0.13.9.json` 保存相同 13 条迁移的历史标签清单，但 `v0.13.9` 发布不完整，因此该文件不代表公开升级基线；已发布 `v0.13.10.json` 继续冻结完全相同的 13 条迁移。已发布 `v1.0.0.json` 在这 13 条之后追加并冻结账号注销、Outbox durable completion 与邮件 attempt fencing 三条迁移，共 16 条；`v1.0.1.json` 与当前 `v1.0.2.json` 继续冻结完全相同的 16 条迁移，证明补丁没有新增或重写 DDL。校验器强制要求与当前 `package.json` 版本同名的清单存在，并完整覆盖仓库当前迁移全集。执行：

```bash
pnpm migration:verify
```

校验会拒绝删除、重排或修改任何已冻结迁移；当前版本清单还必须覆盖所有追加迁移。运行时迁移预检进一步要求数据库的成功记录是这 16 条身份的 checksum 精确连续前缀；没有 `runtime.initialized` 的部分首装可以继续，已初始化实例必须至少完整匹配 13 条 `v0.13.10` 基线。候选失败记录只有在名称、冻结 checksum、前缀位置和当前 PostgreSQL schema marker 全部一致时才会提示或接受受限 rolled-back 恢复。该静态门槛不能替代真实升级；`v0.13.10` 主线与标签流水线已经从精确 `v0.13.8` 镜像创建持久数据、备份并升级到发布镜像。
...
每日定时、手动和正式标签流水线会在 amd64 候选镜像上执行真实 Compose 故障演练：
...
日常主分支提交不执行这组慢速故障演练；它在 amd64/arm64 上执行 Web/Worker/数据库/Redis/首次管理员基础镜像冒烟，并在 amd64 额外执行当前公开基线到候选的验收升级。每个架构在冒烟前固定运行时 Digest，成功后上传同时绑定候选顶层 Digest 的短期 artifact。两个架构和升级门槛成功且提交仍是远程 HEAD 后，工作流只从这些内容地址合并 `edge` 与不可变 `sha-*`；正式标签合并候选顶层 index，并验证运行时平台、全部描述符和 SBOM/provenance attestation 关联均未丢失。主线不移动 `latest`。候选仍必须先由定时或手动运行取得空卷恢复和故障注入证据，正式标签再重跑全部深度门槛。生产环境的托管 PostgreSQL、Redis、SMTP 和 S3 仍需部署者按运行手册完成实例级告警和恢复演练。
...
1. 拉取公开升级基线指定的精确 `ghcr.io/xwordsman/nextbuf:<基线版本>` 镜像；`v1.0.2` 当前为 `1.0.1`。
...
5. 验证目标版本接口、管理员、安装完成状态、上述持久内容、历史编辑字段 `NULL/NULL`、完整 16 条迁移集合和 `runtime.initialized` 目标版本；确认既有 `ProcessedJob.completed_at` 已回填 Outbox `processed_at`，`outbox_events_recovery_pending_idx` 为 valid/ready，邮件 attempt 字段与结果未知状态约束有效。
6. 再执行完整 doctor；流水线结束后销毁临时 Registry、Compose 卷和备份。
7. 验证注销到期后的墓碑、认证与私人数据清理、头像/附件回收 Outbox、管理员角色交接和 Doctor 连续性状态；恢复到最终化前的备份后，在 Worker 清完到期注销积压前不重新开放公网写入。
8. 建立已发布但未处理的 Outbox 后清空 Redis，验证维护任务自动重入队；执行人工 replay 并再次清空 Redis，验证旧 replay 记录不阻断恢复，而新的最终失败重新等待人工 replay。
9. 阻塞邮件投递事务并并发执行到期注销，验证统一的 `EmailDelivery -> WorkerJobFailure -> OutboxEvent` 锁序没有死锁、重复发送或迟到失败回写。

候选镜像的空卷恢复由定时、手动和正式标签流水线的 `nextbuf-backup-v1` 恢复测试覆盖。迁移成功后仍不承诺旧代码直接读取新 Schema；失败恢复遵循 ADR-0015。

正式标签、镜像和 Release 全部成功后，后续开发提交必须把工作流的公开升级基线提升到该已发布版本；不能在标签验证前提前提升。`v1.0.1` 标签验证已经通过，`v1.0.2` 工作流公开升级基线现为 `v1.0.1`。
...
## 15. `v0.13.8` 编辑器与私人草稿阻断项

邀请测试发现的原始风险不是“草稿标签显示错误”，而是发布、自动保存和网络响应可能乱序。`v0.13.8` 按 [ADR-0019](./adr/0019-editor-autosave-idempotency-and-draft-privacy.md) 关闭以下阻断项：

| 风险 | 修复合同 | 发布证据 |
| --- | --- | --- |
| 首次自动保存并发创建多个 Topic | 作者/key 唯一、用户行锁、同 revision 同快照幂等 | 8 路并发 PostgreSQL 集成测试、唯一索引有效性检查 |
| 发布后迟到保存留下草稿或覆盖正文 | 客户端单写队列/发布屏障；服务端 revision 与终态 | 挂起 autosave 的 Playwright、低/同 revision 冲突集成测试 |
| 清空回复草稿后旧请求复活内容或恢复循环 | `cleared` revision 终点；新会话接管后旧会话 `superseded`；恢复到该终态时清除旧 history 并加载规范 Topic 页面状态 | 清空、迟到、接管、发布状态机集成测试；过期 history 浏览器回归 |
| 发布响应丢失后重复回复 | 超时、连接丢失或响应体无效的 2xx 均显示结果未知并保留原 key；恢复接口解析 `published` Post | Playwright 让服务端提交后返回无效 200 JSON，刷新后恢复且仅有一个楼层 |
| 上传完成时切预览或继续输入导致丢字 | 同步正文 ref、无 textarea 时安全插入并返回编写页 | Playwright 挂起上传、继续输入、切预览后正文与附件均保留 |
| 空正文或新 key 造成会话表无界增长 | 每用户滚动一小时最多 60 个新回复编辑会话；`cleared`/`superseded` 保留 30 天后由 Worker 每批最多清理 500 条；`active`/`published` 永不参与该清理 | 第 60/61 个新会话限流集成测试；过期/未过期及四状态 Worker 清理矩阵 |
| Topic 关闭或账号受限后无法收回草稿 | 作者删除自己的既有回复 Draft 和附件引用不因发布权限丧失而失效；站点 `repliesEnabled=false` 只阻止正式发布，仍允许保存和编辑草稿 | 关闭 Topic/受限账号的既有草稿删除矩阵；`repliesEnabled=false` 下草稿保存成功而正式发布被拒绝 |
| 管理员读取或枚举私人草稿 | 私人谱系统一过滤列表、计数、详情和审计；数字编号内容修改/删除/恢复/管理统一伪装为 404 | draft/deleted-from-draft/NULL/未知来源单元、PostgreSQL 与真实 HTTP 矩阵 |
| 异常回复状态或历史引用泄露正文 | 公开列表排除 draft；新引用只接受 published；hidden/deleted 历史引用按权限显示正文或安全占位 | PostgreSQL 引用创建、保留、普通用户与版主可见性矩阵 |

发布门槛（`v0.13.8` 已通过）：

1. `v0.13.8` 迁移清单哈希冻结，三组 key/revision CHECK、回复会话 CHECK 和唯一索引均 valid/ready。
2. GitHub Actions 完整 check、真实 PostgreSQL/Redis/Mailpit、Playwright 和 amd64/arm64 基础镜像冒烟通过；集成测试必须覆盖第 61 个新会话被限流、只清理过期墓碑、Topic 关闭/账号受限后仍可删除既有草稿，以及 `repliesEnabled=false` 下可保存草稿但不能发布；浏览器测试必须覆盖无效 2xx 的结果未知恢复与 `superseded` history 清理。
3. 当时的公开 Beta 流水线要求迁移有差异的 `main` 提交在更新滚动 `latest` 前完成精确 `v0.13.7 -> v0.13.8` 备份/升级，保留既有管理员、节点、主题、首帖、回复、回复草稿、附件引用和 `NULL/NULL` 历史行；该历史门槛已经通过。当前稳定发布通道由 ADR-0020/0022 接管：日常 `main` 在更新 `edge` 前执行 amd64 公开基线到候选的验收升级；空卷恢复和故障注入在定时、手动及正式标签运行中执行，这些深度通道同时重跑升级；只有完整稳定 Release 更新 `latest`。
4. 正式 `v0.13.8` 标签标记为 Pre-release；迁移成功后不直接切回 `v0.13.7`，失败恢复遵循 ADR-0015。
5. 在真实 nextbuf.com 环境重跑验收模板中的快速输入、上传中发布、响应未知刷新、旧 history 接管、发布权限变化后的草稿保存/删除边界、后台草稿隐私和异机恢复步骤。
