...
提交 `68c4a9b` 的 CI #56 已在 amd64 候选镜像上完成上述全部故障注入、恢复后完整 doctor 和 `nextbuf-backup-v1` 空卷恢复。正式标签仍须重跑相同 amd64 门槛，并通过 arm64 首次安装冒烟后才能发布多架构 manifest。
...
CI #56 已真实完成以上六步，并验证升级后的完整 doctor。正式标签发布必须再次通过该升级门槛；候选成功不替代标签镜像、manifest、SBOM、provenance、归档和 GitHub Release 验证。
