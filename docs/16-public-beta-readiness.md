...
| 从受支持 Beta 升级、备份和恢复 | 候选验证 | `v0.12.0 -> v0.13.0` 镜像升级、升级前备份与候选镜像空卷恢复 |
...
- 为诊断容器注入 `/proc` 下的不可写本地存储目标后，`nextbuf doctor` 必须失败并把问题归因到 `storage`；恢复正常配置后诊断必须通过。
...
正式 `v0.13.0` 标签的 amd64 流水线额外执行首个受支持 Beta 升级：
