...
最终 Beta 候选为 `v0.13.9`：它不新增迁移或产品功能，集中关闭可信代理、隐藏节点附件、依赖公告、非 Docker 归档身份、备份一致性和失败升级语义。候选必须从精确 `v0.13.8` 镜像保留真实管理员、主题、回复、草稿和附件，完成空卷恢复、故障注入、双架构镜像与解压归档启动后才发布；具体升级和恢复边界见 [发布说明](./18-v0.13.9-release-notes.md)。
