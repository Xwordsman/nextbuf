...
- 当前发布阻断：提交 `894d1257e579a046b7e6cfadc081b1be66561566` 的主线 [Run 30987867629](https://github.com/Xwordsman/nextbuf/actions/runs/30987867629) 已完整成功；同提交的发布演练 [Run 30988699544](https://github.com/Xwordsman/nextbuf/actions/runs/30988699544) 在 amd64 宝塔初始编排的 `AUTH_SECRET` 保真比较阻断。上一轮修复已让 `yamlValue` 真正产生 `$$`，但测试随后又把它放入 JavaScript `String.replace` 的替换字符串，第二层替换再次把 `$$` 消耗为 `$`；Compose 因而对 `NAME`/`OLD` 输出 24 条未设置警告。进一步核对 Compose 2.38.2 还确认 `config --format json` 会在序列化后把每个 `$` 加倍，正式导出 helper 必须先撤销这一层输出转义再与 Docker inspect 比较。当前待验证修复覆盖两层问题；新的完整主线和发布演练成功前，`v1.0.0` 继续保持 `NO-GO`
- 下一动作：先提交并推送当前宝塔配置保真修复，等待完整主线成功后对同一 commit 重新运行发布演练；随后把通过全部门槛的 `main` HEAD 及其不可变、带证明的 `sha-<提交>` 镜像冻结为唯一正式候选，并在受限验收目录记录 commit、CI、OCI Digest、归档/SBOM artifact。按正式版人工验收模板完成全新安装，并在生产维护窗口通过 `nextbufctl backup --baota` 把实际宝塔实例导出、以 `--keep-stopped` 恢复到 Provider 已隔离的受控副本。2026-08-03 对 `www.nextbuf.com` 的公开只读核对显示线上仍为 `0.13.8`，因此该副本必须先按已发布路径完成 `0.13.8 -> 0.13.10`；预标签候选再按运行手册把 `0.13.10` 与候选完整 OCI index 从记录的 Digest 复制到 loopback Registry，正常拉取并执行 `0.13.10 -> 1.0.0`，逐项证明用户、UID、内容、Better Auth 凭据/Session、附件、Provider、备份恢复和双管理员连续性。单纯本地 tag、忽略 pull 或另一次构建都不构成候选。人工验收后不再提交源码变更，直接在同一候选 commit 创建标签。生产实例不承担破坏性演练，`v1.0.0` 仍未发布，不混入 `v1.1.0` 功能
...
- 稳定版镜像通道修订（`v1.0.0` 稳定化）：完整检查、原生 amd64/arm64 基础镜像冒烟及 amd64 公开基线到候选的验收升级通过后，主分支只从测试回执记录的带证明内容 Digest 更新 `edge` 并保留不可变 `sha-<提交>`，写入 `edge` 前再次核对远程 HEAD；演练和正式标签复用这些源索引，SemVer 与人工验收 `sha-*` 的 OCI index Digest 必须相同。Registry 查询不明确时不得创建或覆盖标签。`latest` 只在最新无后缀 `v1+` 稳定标签的 GitHub Release 完成回执、最终标签 commit、OCI index/平台 Digest、Release 正文 SHA-256、资产 SHA-256 与归档校验和经首次上传后及提升前重新验证时更新；每个标签在共享锁外独立完成并验证回执，只有后续全局最高稳定 Release 调和进入跨标签串行锁。提升使用回执绑定的 `image@sha256:...` 并在写入后复核，当前通道版本和平台成员继续阻止倒退。`v0.x`/预发布不更新。日常主线不重复只属于定时、发布演练和标签运行的空卷恢复与故障注入；演练不写公开发布状态，正式标签重跑相同候选的深度门槛。GitHub active Ruleset `20172767` 已保护 `refs/tags/v*` 的创建、更新和删除，并且只允许仓库所有者 `Xwordsman` bypass。决策见 [ADR-0020](./adr/0020-stable-release-channels-and-lifecycle.md)。
...
- 已记录的自动化代码基线 commit `1054a4de3ee2f6c2be2a72708ae842b3f7d16134` 在同一主线运行通过 Prisma generate、生产依赖审计、迁移清单、Compose/Shell 策略、Prettier、ESLint、TypeScript、124 项单元测试、91 项 PostgreSQL/Redis/Mailpit 真实服务集成、Next.js/Worker/CLI 生产构建和 14 项 Playwright。迁移集成从真实 13 条 `v0.13.10` 不可变历史继续应用 3 条候选迁移，并完成 16 条 `v1.0.0` 清单、HMAC 脱敏快照和允许变换比较；本地同样通过 16 条迁移清单、124 项单元及生产构建。本机无 Docker、PostgreSQL 与 Redis，本地结果不替代 Actions 的真实服务和镜像证据。后续文档提交不会被冒充为同一镜像；最终候选以冻结后 `main` HEAD 的独立 CI 和不可变 `sha-*` 回执为准。
