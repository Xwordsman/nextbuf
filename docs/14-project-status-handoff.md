...
- 当前发布阻断：提交 `935a798c1fa46f56a5a5be7a1ec2b04139f350af` 的主线已完整成功，但发布演练 [Run 30985333056](https://github.com/Xwordsman/nextbuf/actions/runs/30985333056) 发现宝塔 Compose 与导出配置中的 `$` 保真缺陷；JavaScript 替换字符串把 `$$` 当作单个 `$`，测试夹具和正式导出工具都没有真正执行加倍。修复后的提交必须重新通过完整主线和发布演练，`v1.0.0` 继续保持 `NO-GO`
- 下一动作：先完成上述宝塔配置保真修复及完整主线/发布演练；随后把通过全部门槛的 `main` HEAD 及其不可变、带证明的 `sha-<提交>` 镜像冻结为唯一正式候选，并在受限验收目录记录 commit、CI、OCI Digest、归档/SBOM artifact。按正式版人工验收模板完成全新安装，并在生产维护窗口通过 `nextbufctl backup --baota` 把实际宝塔实例导出、以 `--keep-stopped` 恢复到 Provider 已隔离的受控副本。2026-08-03 对 `www.nextbuf.com` 的公开只读核对显示线上仍为 `0.13.8`，因此该副本必须先按已发布路径完成 `0.13.8 -> 0.13.10`；预标签候选再按运行手册把 `0.13.10` 与候选完整 OCI index 从记录的 Digest 复制到 loopback Registry，正常拉取并执行 `0.13.10 -> 1.0.0`，逐项证明用户、UID、内容、Better Auth 凭据/Session、附件、Provider、备份恢复和双管理员连续性。单纯本地 tag、忽略 pull 或另一次构建都不构成候选。人工验收后不再提交源码变更，直接在同一候选 commit 创建标签。生产实例不承担破坏性演练，`v1.0.0` 仍未发布，不混入 `v1.1.0` 功能
