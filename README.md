...
当前版本为 `v0.13.1` 公开 Beta 补丁：在完整社区、管理后台和生产部署基础上，增加安全边界、依赖审计、容量诊断、迁移历史门槛、跨 Beta 升级、故障恢复、多视口无障碍，并修正通用开源发行版的首次安装与节点初始化体验。
...
首次启动后直接访问 `${APP_URL}`，未安装站点会自动跳转到 `/setup`。使用 `.env` 中只显示在服务器上的 `SETUP_TOKEN` 创建首位管理员，登录后在 `/admin/nodes` 创建适合当前社区的节点；发行版不内置特定社区分类。完整宝塔、反向代理、备份恢复与升级步骤见 [安装与运维运行手册](./docs/13-installation-operations-runbook.md)。
