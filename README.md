...
当前版本为 `v0.9.0` 通知、邮件与 Worker 完整链路：回复、提及、关注主题回复和主题管理动作经事务性 Outbox 生成结构化站内通知；用户可以管理未读、归档及普通通知渠道偏好；加密邮件、失败任务、重放请求和带租约的周期调度以 PostgreSQL 为事实来源，并提供仅站点管理员可访问的 Worker 运维摘要。举报治理和持久化信任计算仍未实现。
