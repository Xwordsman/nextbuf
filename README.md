...
当前版本为 `v0.10.0` 举报、治理、角色和信任等级：公开主题、回复和用户可以被举报并按目标聚合为案件；节点版主、全局版主和管理员按独立角色矩阵执行内容处置与制裁，服务端授权直接读取 PostgreSQL 有效制裁；TL0-TL3 使用版本化规则和真实指标自动计算，TL4 仅由管理员人工确认，规则变更先预估再通过可恢复 Worker 分批应用。
