...
当前版本为 `v0.13.2` 公开 Beta 补丁：在完整社区、管理后台和生产部署基础上，增加安全边界、依赖审计、容量诊断、迁移历史门槛、跨 Beta 升级、故障恢复、多视口无障碍，并提供无需 `.env`、后续无需修改版本号的宝塔单文件 Compose。
...
宝塔面板用户可以直接粘贴 [`compose.baota.yml`](./compose.baota.yml)，首次替换其中的域名、密码、应用密钥和 SMTP 配置后启动。该模板使用正式 `latest` 通道，后续升级只需在面板拉取新镜像并重建，不再编辑版本号。
