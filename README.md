...
当前已发布版本为 `v0.13.8` 公开 Beta 补丁，主分支候选为 `v0.13.9` 最终 Beta 加固：保留主题/回复编辑幂等、真实社区数据、搜索、分页、认证流程、既有三栏布局和无需 `.env` 的宝塔单文件 Compose，同时收紧可信代理与隐藏节点附件边界，并强化依赖审计、发布归档、备份恢复和精确升级。`v0.13.9` 验证后进入已批准的 `v1.0.0` 稳定化，不混入 `v1.1.0` 功能。
