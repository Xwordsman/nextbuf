...
当前版本为 `v0.5.0` 用户资料与账号中心：在完整身份认证链路之上，已实现从 1000 起且永不复用的数字 UID、唯一 `@username`、30 天修改冷却、永久历史别名、头像裁剪与本地持久化、公开资料与用户页、隐私设置和 14 天可撤销注销申请。界面当前以 `TL0` 表示初始身份，完整信任等级计算仍规划在 `v0.10.0`。首页节点、话题、概览和在线成员仍是只读演示 ViewModel；主题写入、回复、通知、治理和管理后台尚未实现。
