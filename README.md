...
当前版本为 `v0.8.0` 互动、搜索与内容发现：点赞、主题收藏、用户/主题关注、阅读状态和个人收藏/关注/参与列表以 PostgreSQL 为事实来源；浏览按 30 分钟访问者哈希桶经 Outbox Worker 聚合；热门算法使用有上限的互动信号与时间衰减；标题、Markdown 正文、成员和节点通过 PostgreSQL FTS/`pg_trgm` 搜索。通知、治理后台和持久化信任计算仍未实现。
