...
当前已发布版本为 `v0.13.8` 公开 Beta 补丁：在 `v0.13.7` 官方 shadcn/ui 全站界面基础上，修复主题/回复自动保存与发布竞态、响应丢失恢复和后台私人草稿边界；保留真实社区数据、搜索、分页、认证流程、既有三栏布局和无需 `.env` 的宝塔单文件 Compose。主分支正在完成 `v1.0.0` 稳定版门槛，正式标签前仍按 Beta 升级与备份合同运行。
