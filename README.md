...
当前版本为 `v0.13.7` 公开 Beta 补丁：全部公开前台页面切换到官方 shadcn/ui `radix-nova` 组件体系，保留真实社区数据、搜索、分页、认证流程和既有三栏布局合同；管理后台、授权、审计、设置与部署合同保持不变，并继续提供无需 `.env`、后续无需修改版本号且使用固定容器名的宝塔单文件 Compose。
