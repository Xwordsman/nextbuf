...
`v0.13.10` 是最后一个完整发布的公开 Beta；`v1.0.0` 正在稳定化，尚未发布。稳定版支持周期从 `v1.0.0` GitHub Release、不可变镜像和发布资产全部完成时开始。在此之前，`v0.13.10` 只获得阻断稳定版或高影响安全问题的尽力修复，并作为到 `v1.0.0` 的直接升级基线。
...
问题必须能在对应 Release 文档声明的官方组合中复现。V1 计划支持：
