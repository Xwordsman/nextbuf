...
import {
  ...
  assertAdministratorContinuityAfterRoleRevocation,
  assertUserEligibleForAdministratorRole,
...
export async function grantCommunityRole(input: {
  ...
  return getPrismaClient().$transaction(async (transaction) => {
    ...
    if (input.role === "admin") {
      ...
      if (concurrentlyGranted) return concurrentlyGranted;
    }
...
export async function revokeCommunityRole(input: {
  ...
  await getPrismaClient().$transaction(async (transaction) => {
    ...
    if (assignment.role === "admin" && assignment.scopeKey === "site") {
      await assertAdministratorContinuityAfterRoleRevocation(transaction, assignment.userId);
    }
