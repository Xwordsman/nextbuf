...
export async function startWorker(): Promise<void> {
  ...
  let workerResources: ReturnType<typeof createOutboxWorker> | undefined;
  ...
  try {
    ...
    workerResources = createOutboxWorker();
    const { worker, connection } = workerResources;
    ...
    dispatchTimer = setInterval(() => void dispatch(), environment.OUTBOX_POLL_INTERVAL_MS);
    dispatchTimer.unref();
    await dispatch();
    ...
    logger.info("NextBuf worker is ready", {
      workerId,
    ...
    await Promise.allSettled([
      activeDispatch ?? Promise.resolve(),
      activeSchedule ?? Promise.resolve(),
    ]);
    if (workerResources) {
      await Promise.allSettled([workerResources.worker.close(true)]);
      if (workerResources.connection.status !== "end") {
        await Promise.allSettled([workerResources.connection.quit()]);
      }
    }
    await Promise.allSettled([
      closeSystemQueue(),
    ...
    await Promise.allSettled([disconnectRedisClient(), disconnectPrismaClient()]);
    throw error;
  }
}
