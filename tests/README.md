...
当前未发布的 `v1.0.0` 候选包含 109 项单元测试、87 个真实服务集成 case 和 14 个 Playwright 旅程，并新增最终账号注销、治理证据/邮件隐私、管理员连续性、首次安装 claim 所有权与当前表单密码证明、Better Auth 资料更新边界、replay 与普通 Dispatcher 的终态 Redis Job 竞态，以及 `v0.13.10 -> v1.0.0` 升级恢复门槛。迁移策略单测覆盖冻结历史的合法前缀、checksum 漂移、缺口/额外记录和自定义 PostgreSQL schema。只有 GitHub Actions 和人工验收全部完成后，这些候选数量才可写入正式发布证据。
...
Pull Request 只执行完整代码/真实服务/E2E 检查；主分支额外使用原生 amd64/arm64 构建、拉取和基础冒烟。主分支与正式标签只在 Linux x64 Runner 构建并解压冒烟 Linux x64 归档，校验 checksum、Next.js standalone 的 pnpm 链接不悬空或逃出归档、关键运行依赖可解析、systemd/PM2 路径合同，并在真实 PostgreSQL、Redis 和 Mailpit 上从归档执行 setup、启动独立 Web/Worker、创建首位管理员和运行 doctor；失败摘要会标明具体阶段并脱敏。每个架构用 artifact 固定实际测试的运行时与候选顶层 Digest；只有双架构镜像和发布归档同时成功，主分支才从内容地址合并 `sha-<提交>` 与滚动 `edge`。正式标签从候选顶层 Digest 合并不可变 SemVer manifest，并核对 attestation；Release 资产完成后才更新 `latest`。复用完成状态或提升前会下载完成回执与三项必需资产，核对标签目标 commit、版本、全部 SHA-256 和旁路归档校验和。每日定时、手动和 `v*` 标签运行的 amd64 额外执行空卷恢复、故障注入和公开版本到候选的真实升级；日常主分支不重复这些深度演练。
