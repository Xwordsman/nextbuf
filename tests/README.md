...
`v1.0.0` 已通过 124 项单元、91 项真实 PostgreSQL/Redis/Mailpit 集成、14 项 Playwright、双架构镜像、归档、恢复、故障注入和精确 `v0.13.10 -> v1.0.0` 升级门槛，并于 2026-08-06 完整发布。`v1.0.1` 在相同基线上增加隐藏节点角色矩阵、可信 Origin 规范化、Worker 启动失败清理和已提交邀请注册计数回归；CI 使用独立无迁移脚本直接验证 `v1.0.0 -> v1.0.1`，历史三迁移故障脚本仍保留 `v0.13.10 -> v1.0.0` 默认基线。升级前至少保留 1 位可接管管理员，正式发布默认 2 位；若项目所有者对具体版本批准单管理员风险豁免，必须保留冗余警告，0 位始终阻断。
