...
已审计自动化基线 `9342815e394b4e93d215ace98b2937412f422016` 包含 109 项单元测试、87 个真实服务集成 case 和 14 个 Playwright 旅程，并覆盖最终账号注销、治理证据/邮件隐私、管理员连续性、首次安装 claim 所有权与当前表单密码证明、Better Auth 资料更新边界、replay 与普通 Dispatcher 的终态 Redis Job 竞态，以及 `v0.13.10 -> v1.0.0` 升级恢复门槛。当前升级证据候选 `1054a4de3ee2f6c2be2a72708ae842b3f7d16134` 已在主线 CI 通过 124 项单元、91 项真实服务集成和 14 项 Playwright：新增严格 HMAC 快照合同，以及用真实 13 条历史迁移和 3 条候选迁移执行的验收比较回归。迁移策略单测继续覆盖冻结历史的合法前缀、checksum 漂移、缺口/额外记录和自定义 PostgreSQL schema。人工验收和正式标签资产完成前，这些数量仍只构成候选自动化证据。
