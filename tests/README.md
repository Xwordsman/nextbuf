...
历史 `v0.13.10` 最终 Beta 声明 77 项单元测试、47 个真实服务集成 case 和 14 个 Playwright spec；它保留为到 `v1.0.0` 的历史升级来源，不再是当前补丁升级基线。

历史自动化基线 `9342815e394b4e93d215ace98b2937412f422016` 包含 109 项单元、87 项真实服务集成和 14 项 Playwright；后续代码基线 `1054a4de3ee2f6c2be2a72708ae842b3f7d16134` 通过 124 项单元、91 项真实服务集成和 14 项 Playwright，并使用真实 13 条历史迁移和 3 条追加迁移执行严格 HMAC 验收比较。它们保留为标签前历史证据；最终 `v1.0.0` commit `e2a58209ae09caa0e785cca9ad4a05b2d822c384` 的主线、演练和标签流水线已经独立通过。

`v1.0.0` 已通过 124 项单元、91 项真实 PostgreSQL/Redis/Mailpit 集成、14 项 Playwright、双架构镜像、归档、恢复、故障注入和精确 `v0.13.10 -> v1.0.0` 升级门槛，并于 2026-08-06 完整发布。`v1.0.1` 增加隐藏节点角色矩阵、可信 Origin 规范化、Worker 启动失败清理和已提交邀请注册计数回归，并于 2026-08-07 完整发布。`v1.0.2` 新增请求内去重、轻量主题 Shell、Profile 只读、共享账号 layout、局部 loading 和软导航回归；CI 使用独立无迁移脚本直接验证 `v1.0.1 -> v1.0.2`，历史三迁移故障脚本仍保留 `v0.13.10 -> v1.0.0` 默认基线。升级前至少保留 1 位可接管管理员，正式发布默认 2 位；若项目所有者对具体版本批准单管理员风险豁免，必须保留冗余警告，0 位始终阻断。
