...
`v0.4.0` 已覆盖环境配置、迁移 readiness、setup 幂等、Outbox 投递、Worker 重启续跑、任务幂等、Redis 清空后的事实数据完整性、scrypt 凭证、验证标识 HMAC、邮件 AES-GCM、邀请码原子使用、SMTP Worker、注册/验证/登录、两设备会话和密码重置撤销。
